 ...
 * Also bridges the existing .save / Save-this-destination UI.
 */
(function(){
  'use strict';
  ...
  function write(items){
    ...
    try{window.dispatchEvent(new CustomEvent('exploreup:favorites-changed',{detail:items}));}catch(e){}
    return true;
  }
  window.ExploreUPFavorites={
    all:()=>read(),
    has:(name,city='')=>read().some(x=>x.name===String(name)&&x.city===String(city||'')),
    toggle:(name,type='Place',city='')=>{
      name=String(name||'').trim(); city=String(city||'').trim(); type=String(type||'Place').trim()||'Place';
      if(!name)return false;
      const items=read();
      const i=items.findIndex(x=>x.name===name&&x.city===city);
      if(i>=0){items.splice(i,1);write(items);return false}
      items.push({name,type,city});write(items);return true;
    },
  ...
  function wire(){
    document.querySelectorAll('[data-favorite]').forEach(btn=>wireButton(btn,()=>({
      name:btn.dataset.favorite||btn.textContent.trim(),type:btn.dataset.favoriteType||'Place',city:btn.dataset.favoriteCity||currentCity()
    })));
    document.querySelectorAll('.save').forEach(btn=>{
      wireButton(btn,()=>({name:inferLegacyName(btn),type:'Place',city:btn.dataset.favoriteCity||''}));
    });
    ...
    if(bar&&!bar.dataset.favoritesBarWired){
      bar.dataset.favoritesBarWired='1';bar.onclick=null;bar.textContent='♥ Favorites ';const count=document.createElement('span');count.id='v21FavCount';bar.appendChild(count);bar.addEventListener('click',()=>{location.href='./favorites.html'});
    }
    updateFavUI();
  }
