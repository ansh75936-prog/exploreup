 ...
 * V21 Nearby back-navigation fix: restore normal page scrolling after returning from Maps.
 */
(function(){
  'use strict';
  ...
  function addFeedbackLink(){
    ...
    if(!host||host.querySelector('[data-exploreup-feedback]'))return;
    const a=document.createElement('a');
    a.href='./feedback.html';
    a.dataset.exploreupFeedback='1';
    a.textContent='💬';
    a.title='Give Feedback';
    a.setAttribute('aria-label','Give Feedback');
    a.style.cssText='width:38px;height:38px;border:1px solid #52708f;border-radius:50%;display:grid;place-items:center;color:#fff;text-decoration:none;font-size:18px;background:transparent';
    host.appendChild(a);
  }
  ...
  function restoreAfterMaps(){
    const html=document.documentElement,body=document.body,modal=document.getElementById('modal');
    if(modal){modal.style.display='none';modal.setAttribute('aria-hidden','true');modal.classList.remove('show','open','active');}
    [html,body].forEach(el=>{
      ...
      el.classList.remove('modal-open','no-scroll','lock-scroll','scroll-locked','page-lock','locked');
    });
  ...
  document.addEventListener('DOMContentLoaded',wire);window.addEventListener('load',wire);window.addEventListener('pageshow',restoreAfterMaps);window.addEventListener('popstate',restoreAfterMaps);document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')restoreAfterMaps()},{passive:true});window.addEventListener('exploreup:favorites-changed',updateFavUI);
})();
