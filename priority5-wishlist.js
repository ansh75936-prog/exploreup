...
const use=box.querySelector('#p5wlUse'),clear=box.querySelector('#p5wlClear');if(use)use.onclick=useWishlistInTrip;if(clear)clear.onclick=clearPlaces}
...
function renderInPlan(){const r=document.getElementById('planResult');if(!r)return;const selected=getSelected(),key=selected.join('\u0001');const old=r.querySelector('.p5wishlist-plan');if(!selected.length){if(old)old.remove();return}if(old&&old.dataset.key===key)return;if(old)old.remove();const box=document.createElement('div');box.className='p5box p5wishlist-plan';box.dataset.key=key;box.innerHTML='<b>🧳 Wishlist stops</b><div class="p5muted">These saved places were selected by you. Fit them into the itinerary where practical; they are not guaranteed route or timing data.</div><div class="p5chips">'+selected.map(x=>'<span class="p5chip">'+esc(x)+'</span>').join('')+'</div>';r.appendChild(box)}
let scheduled=false;
function observe(){if(scheduled)return;scheduled=true;setTimeout(()=>{scheduled=false;injectStyle();renderBridge();renderInPlan()},50)}
