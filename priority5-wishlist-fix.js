...
function render(){const r=document.getElementById('planResult');if(!r)return;const selected=get();const old=r.querySelector('.p5wishlist-plan');if(old)old.remove();if(!selected.length)return;const box=document.createElement('div');box.className='p5box p5wishlist-plan';box.innerHTML='<b>🧳 Wishlist stops</b><div class="p5muted">Your saved places will be treated as optional stops inside the selected district plan. They do not replace the destination or provide guaranteed route/timing data.</div><div class="p5chips">'+selected.map(x=>'<span class="p5chip">'+esc(x)+'</span>').join('')+'</div>';r.appendChild(box)}
